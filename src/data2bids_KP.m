...
if need_eeg_json
  ...
  % merge the information specified by the user with that from the data
  % in case fields appear in both, the first input overrules the second
  eeg_json = mergeconfig(eeg_settings,     eeg_json);
  eeg_json = mergeconfig(generic_settings, eeg_json);
...
if need_coordsystem_json
  ...
  % merge the information specified by the user with that from the data
  % in case fields appear in both, the first input overrules the second
  coordsystem_json =  mergeconfig(coordsystem_settings, coordsystem_json); % FIXME the order of precedence is different here
  ...
  if eval(sprintf('need_%s_json', modality{i}))
    ...
    switch cfg.writejson
      ...
      case 'replace'
        ft_write_json(filename, modality_json);
      case 'merge'
        ft_write_json(filename,  mergeconfig(modality_json, existing))
      case 'no'
        % do nothing
